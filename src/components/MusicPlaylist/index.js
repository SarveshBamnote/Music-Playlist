...
class MusicPlaylist extends Component {
  ...
  onClickDelete = id => {
    ...
    const listAfterDelete = songsList.filter(each => each.id !== id)
    this.setState({songsList: listAfterDelete})
  }

  render() {
    const {songsList, searchInput} = this.state

    const searchList = songsList.filter(each =>
      each.name.toLocaleLowerCase().includes(searchInput.toLowerCase()),
    )

    const listLenght = searchList.length
